"""
daemon_launcher.py - Frozen daemon entry point for MoltBot on-device.
...
_DEFAULTS = {
    'SOVEREIGN_STORE_DIR': '/data/data/com.moltbot/files/sovereign_memory',
    'APP_FILES_DIR':       '/data/data/com.moltbot/files',
    'DOTENV_PATH':         '/data/data/com.moltbot/files/.env',
...
def launch():
    ...
    t1 = threading.Thread(target=_run_server, args=('server', 8001),       daemon=True, name='moltbot-fastapi')
    t2 = threading.Thread(target=_run_server, args=('pythonserver', 8765), daemon=True, name='moltbot-pyserver')
    t1.start()
    t2.start()
