...
# Unified reasoning engine (L1+L4+L11) — replaces separate skill files
try:
    from skills.reasoning import get_reasoning as _get_reasoning
    _reasoning_engine = _get_reasoning()
except Exception:
    _reasoning_engine = None

# Keep legacy aliases so anything else that references them still works
_intent_predictor = _reasoning_engine
_cognitive_mgr    = _reasoning_engine
...
db        = client[os.environ.get('DB_NAME', 'moltbot_local')]
...
sovereign_mem = SovereignMemoryProd(session_id="moltbot-prime")

# Cognitive Cell — 8-phase reasoning loop wrapping sovereign memory
cognitive_cell = SovereignCognitiveCell(cell_id="moltbot", session_id="moltbot-cognitive")
...
# Seed tools that MoltBot starts with
SEED_TOOLS = [
...
SYSTEM_PROMPT = """You are MoltBot (Snowball Agent) — a resourceful, self-aware AI assistant.
...
- Your BODY is the user's PHONE (the MoltBot app executes device_action commands directly)
...
6. NO EXTERNAL BRIDGES: The MoltBot app IS the bridge to the phone!
...
async def process_agent_action(action_data: Dict, conversation_id: str, settings: Settings) -> Dict:
    ...
    if action_type == "execute":
        ...
        try:
            ...
            # Save to file
            export_path = f"/tmp/moltbot_brain_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.json"
...
Note: requires Edge launched with --remote-debugging-port=9222"""
    ...
    # ── Unified Reasoning Engine (L1 + L4 + L11) ─────────────────────────────
    reasoning_modifier = ""
    try:
        if _reasoning_engine:
            analysis = _reasoning_engine.analyze(user_message, history[-10:])
            reasoning_modifier = analysis.get("system_modifier", "")
    except Exception:
        pass

    # Append reasoning modifier to system prompt
    effective_system = system_prompt + reasoning_modifier
    ...
    for iteration in range(max_iterations):
        ...
        try:
            ...
            if '{"action"' in response:
                ...
                if _steward:
                    try:
                        gate = _steward.evaluate_action("moltbot", action_data)
    ...
    if final_response and _push_tokens:
        ...
        asyncio.create_task(_send_push("MoltBot", preview))
...
async def get_skills():
    ...
    return {
        ...
        "reasoning_active": {
            "engine_l1_l4_l11": _reasoning_engine is not None,
...
async def startup_healing():
    ...
    await healer.track_evolution("system_boot", "MoltBot self-healing engine initialized")
