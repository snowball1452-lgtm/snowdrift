...
import com.snowdrift.databinding.ActivityMainBinding
...
class MainActivity : AppCompatActivity() {
    ...
    private fun updateServiceStatus() {
        val isRunning = SnowDriftAccessibilityService.isRunning
        binding.accessibilityCard.visibility = if (isRunning) View.GONE else View.VISIBLE
        binding.statusText.text = if (isRunning) "SnowDrift is active \uD83D\uDFE2" else "Service not running"
    }
    
    private fun openAccessibilitySettings() {
        val intent = Intent(Settings.ACTION_ACCESSIBILITY_SETTINGS)
        startActivity(intent)
        Toast.makeText(this, "Enable SnowDrift in Accessibility settings", Toast.LENGTH_LONG).show()
    }
